   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +60177637943")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +60177637943 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
"sourceUrl": `${wagc}`}}})
                	...
                	const xeonbuffer = await getBuffer(ppuser)
                    const xeontime = moment.tz('Asia/Kuala_Lumpur').format('HH:mm:ss')
	                const xeondate = moment.tz('Asia/Kuala_Lumpur').format('DD/MM/YYYY')
                	let xeonName = num
